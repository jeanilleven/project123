     ...
     <div class="main-sidebar sidebar-collapse navbar-collapse collapse" v-bind:class="hide" id="idfactory" >
      <div class="sidebar">
        <ul class="sidebar-menu">
            ...
            <li v-for="item, index in menu" v-bind:class="{ 'active-menu': item.flag === true }" v-on:click="setActive(index)" v-if="(((item.users === user.type || item.users === 'ALL') && user.type !== 'ADMIN' || user.type !== 'MARKETING') || user.type === 'ADMIN' || user.type === 'MARKETING') && menuFlag === true" class="menu-holder">
